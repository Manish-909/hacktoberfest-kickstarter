#!/usr/bin/env python3
"""
Final Resume Processor using Gemini API
...
class FinalResumeProcessor:
    """Final service for processing resume files using Gemini API"""
...
"""
    ...
    def parse_gemini_response(self, response_text: str) -> Dict:
        ...
        try:
            print(f"Parsing response: {response_text[:200]}...")
    ...
    def extract_json_from_text(self, text: str) -> Dict:
        """Extract JSON from text using a more flexible approach"""
        import re
        ...
        for match in matches:
            try:
                return self.validate_extracted_profile(json.loads(match))
            except:
                continue
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python final_resume_processor.py <resume_file_path>")
    ...
    try:
        ...
        processor = FinalResumeProcessor()
